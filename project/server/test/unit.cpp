...
#include"../include/async.http.server.h"
...
TEST(DataBaseEnvironment, check_user)
 {
...
TEST(SERVER, add_user)
 {
//   std::size_t num_threads = boost::lexical_cast<std::size_t>(4);
//   auto s = std::make_shared<server3::server>("localhost", "8080", num_threads);
...
TEST(SERVER, change_user_status)
 {
//   std::size_t num_threads = boost::lexical_cast<std::size_t>(4);
//   auto s = std::make_shared<server3::server>("localhost", "8080", num_threads);
