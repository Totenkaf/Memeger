...
pqxx::result Postgre_DB::select(const std::string& table, std::string where = std::string(), 
    ...
    if (what.size() > 0) {
        ...
        request = request.substr(0, request.size() - 2); /*MAGIC NUMBER. TO DO SMART*/
    }
    else {
        request += "*";
    }
...
std::vector<TextMessage> Postgre_DB::get_last_N_messages_from_chat(const Chat& chat, int num_of_messages = -1) {
    std::vector<TextMessage> messages;
    std::string where = "chat_id = '" + remove_danger_characters(chat.get_chat_id()) + "'";
    std::vector<std::string> what = {"id", "user_from", "chat_id", "content", "is_read"};
