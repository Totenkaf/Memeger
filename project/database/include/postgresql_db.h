...
class Postgre_DB : public IDataBase {
 private:
  ...
  virtual int insert(const std::string& table,
                     const std::vector<std::string>& table_fields,
                     const std::vector<std::string>& values,
                     std::vector<std::string>& output_params) override;
  ...
  std::string parse_output_params(
 ...
 public:
  ...
  bool find_chat_by_chat_name(
      const std::string& chat_name);  // по идее, константный
  Chat get_chat_by_chat_name(
      const std::string& chat_name);  // по идее, константный
  Chat get_chat_by_id(const std::string& chat_id);  // по идее, константный
  std::vector<std::string> get_participants_from_chat(
      const Chat& chat);  // по идее, константный
  std::vector<Chat> get_all_chats_by_user_login(
      const std::string& login);  // по идее, константный

  int add_user_chat_link(
      const std::string& user_id,
      const std::string& chat_id);  // Возможно добавить в приватную секцию.
};
