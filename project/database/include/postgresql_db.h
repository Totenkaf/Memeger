...
class Postgre_DB : public IDataBase {
 private:
  ...
  virtual int delete_(const std::string& table,
                      const std::string where) override;
  ...
  int init_tables();
  int execution_table(const std::string& table_name,
                      std::shared_ptr<pqxx::connection>& PG_conn);
  ...
  int add_user_chat_link(const std::string& user_id,
 ...
 public:
  ...
  std::vector<TextMessage> get_last_N_messages_from_chat(const Chat& chat,
                                                         int num_of_messages);
  ...
  Chat get_chat_by_chat_name(const std::string& chat_name);
  Chat get_chat_by_id(const std::string& chat_id);
  std::vector<std::string> get_participants_from_chat(const Chat& chat);
  std::vector<Chat> get_all_chats_by_user_login(const std::string& login);
};
